...
Tests use Python `unittest` and are run via the `smv-pytest` tool:

```sh
# Run all unit tests
tox -e 3 -- bash tools/smv-pytest --spark-home .sparks/<spark-version>

# Run a specific test file (by filename without .py)
tox -e 3 -- bash tools/smv-pytest --spark-home .sparks/<spark-version> -t testDataSetMgr

# Filter to a specific test class
tox -e 3 -- bash tools/smv-pytest --spark-home .sparks/<spark-version> -t testDataSetMgr.SomeTestClass
```
